/*****************************************
Diem chuan duoc tao lap theo mang 2 chieu:
+ Chi so cot la khu vuc.
+ Chi so hang la doi tuong.
...
void Nhap1ThiSinh(thisinh &x)
{
	...
	do
	 {
	cout << "Nhap diem Toan: "; cin >> x.toan;
	}
	 while (!(x.toan>=0&&x.toan<=10));
	do 
	{cout << "Nhap diem Ly: "; cin >> x.ly;}
	 while (!(x.ly>=0&&x.ly<=10));
	do 
	{cout << "Nhap diem Hoa: "; cin >> x.hoa;}
...
void XuatThuKhoa(thisinh x[],int n)
{
	...
	float diemmax;
	for(int i=0;i<n;i++)
		if(x[i].tongdiem>x[vt].tongdiem) vt=i;
	diemmax=x[vt].tongdiem;
	for(int i=0;i<n;i++)
		if(x[i].tongdiem==diemmax) Xuat1ThiSinh(x[i]);
}
int main()
{
	thisinh x[50];
